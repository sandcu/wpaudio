			    ...
			    unsigned int *priv_bitlen)
{
  ...
  *priv_bitlen = lsf ? ((nch == 1) ? 1 : 2) : ((nch == 1) ? 5 : 3);
  ...
  for (gr = 0; gr < ngr; ++gr) {
	  struct sideinfo::granule *granule = (struct sideinfo::granule *)(&si->gr[gr]);

    for (ch = 0; ch < nch; ++ch) {
		struct sideinfo::granule::channel *channel = (struct sideinfo::granule::channel *)(&granule->ch[ch]);
...
unsigned int III_scalefactors_lsf(struct mad_bitptr *ptr,
								  struct sideinfo::granule::channel *channel,
								  struct sideinfo::granule::channel *gr1ch, 
								  int mode_extension)
{
...
unsigned int III_scalefactors(struct mad_bitptr *ptr, 
							  struct sideinfo::granule::channel *channel,
							  struct sideinfo::granule::channel const *gr0ch, 
							  unsigned int scfsi)
{
  struct mad_bitptr start;
 ...
 */
static
void III_exponents(struct sideinfo::granule::channel const *channel,
 ...
 */
static
void III_reorder(mad_fixed_t xr[576], 
				 struct sideinfo::granule::channel const *channel,
...
enum mad_error III_stereo(mad_fixed_t xr[2][576],
			  struct sideinfo::granule const *granule,
			  ...
			  unsigned char const *sfbwidth)
{
  ...
  if (header->mode_extension & I_STEREO) {
	  const struct sideinfo::granule::channel * right_ch = &granule->ch[1];
			  ...
			  struct sideinfo *si, unsigned int nch)
{
  ...
  for (gr = 0; gr < ngr; ++gr) {
    struct sideinfo::granule *granule = &si->gr[gr];
    ...
    for (ch = 0; ch < nch; ++ch) {
      struct sideinfo::granule::channel *channel = &granule->ch[ch];
      unsigned int part2_length;
    ...
    for (ch = 0; ch < nch; ++ch) {
      const struct sideinfo::granule::channel *channel = &granule->ch[ch];
...
int mad_layer_III(struct mad_stream *stream, struct mad_frame *frame)
{
  ...
  if (stream->main_data == 0) {
    stream->main_data = (unsigned char (*)[2567])(malloc(MAD_BUFFER_MDLEN));
    if (stream->main_data == 0) {
